...
block = (
    ...
    "    # Match the EXACT requested GPU model AND count: Vast ignores gpu_name and\n"
    "    # num_gpus in the query when geolocation is present, so a 2x-GPU offer would\n"
    "    # otherwise slip through (you'd pay for 2 GPUs; GROMACS uses 1).\n"
    "    _exact = [o for o in instance_list\n"
    "              if str(o.get('gpu_name', '')).strip().lower() == _want\n"
    "              and int(o.get('num_gpus', 0) or 0) == int(num_gpus)]\n"
    "    # Drop known-bad machines (e.g. hosts whose kaalia/OCI shim fails to start\n"
    "    # containers). Set env VAST_BLOCK_MACHINES='24764,12345' to exclude them.\n"
    "    _blk = set(p for p in _os.environ.get('VAST_BLOCK_MACHINES', '').replace(',', ' ').split() if p)\n"
    "    if _blk:\n"
    "        _exact = [o for o in _exact if str(o.get('machine_id', '')) not in _blk]\n"
    "    if _exact:\n"
    ...
    "        try:\n"
    "            _capf = float(_cap)\n"
    "            instance_list = [o for o in instance_list\n"
